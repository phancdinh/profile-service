...
public class ProfileBizService {
    ...
    public BasicInfoDto create(String htId, BasicInfoDto basicInfoDto) throws DataConflictingException {
        ...
        return Optional.of(profileConverterHelper.convert(basicInfoDto, profile.getId()))
                .map(basicInfoDataService::create)
                .map(basicInfo -> profileConverterHelper.convertToDto(basicInfo, htId, BasicInfoDto.class))
                .orElseThrow();
    }

    public BasicInfoDto find(String htId) throws DataNotExistingException {
